...
export function createProcess(processName: string, targetCount?: number) {
    if (existingProcesses.has(processName))
        throw new Error(`Process ${processName} exists already!`);
    ...
    function startProcess(): Promise<string[]> {
        state = 'INPROGRESS';
            ...
            const cycle: ProcessCycleActions = {
                begin: (nameOrId: string) => {
                    ...
                    if (processingUnitsRunning.has(nameOrId)) {
                        ...
                        if (
                            processingUnitsDone.size ===
                                processingUnitsRunning.size ||
                            processingUnitsDone.size === targetCount
                        ) {
                            state = 'DONE';
                            resolve(Array.from(processingUnitsDone));
                        }
                    } else {
                        throw new Error(
                            `Process unit ${nameOrId} tried to be done before it even started.`
                        );
                    }
                },
            };
...
export function isProcessEvent(
    event: JokiEvent
): undefined | [string, ProcessCycleActions] {
